...
//     // let regionConfig = await MessageModel.findOne({ where: { state } });
//     let regionConfig = await MessageModel.findOne({ where: { state } });

//     if (!regionConfig) {
//         console.warn(`No config for state: ${state}, trying default...`);
//         regionConfig = await MessageModel.findOne({ where: { state: "default" } });
//     }
...
//         try {
//             await autoCloneVoice(regionConfig);
//             await regionConfig.reload(); // DB se fresh data lo
//         } catch (err) {
...
// const autoCloneVoice = async (regionConfig) => {
//     console.log("🔵 autoCloneVoice START — region:", regionConfig.state);
//     console.log("🔵 Video path:", regionConfig.video_path);

//     const { cloneVoice } = await import("./elevenLabsService.js");

//     const baseVideoAbs = path.join(ROOT, "public", regionConfig.video_path);

//     // ← YEH ADD KARO — file exist karti hai?
//     console.log("🔵 Video file exists?", fs.existsSync(baseVideoAbs));

//     const audioExtractPath = path.join(ROOT, "public", "uploads", "audio",
...
//     console.log("🎙️ Cloning voice for region:", regionConfig.state);
...
//             audioExtractPath
//         );
//         await regionConfig.update({ elevenlabs_voice_id: voiceId });
...
//     const audioPath = path.join(ROOT, "public", "uploads", "audio",
//         `temp_${dealerId}_${Date.now()}.mp3`);
//     const baseVideoAbsPath = path.join(ROOT, "public", regionConfig.video_path);
